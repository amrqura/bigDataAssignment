...
public class UserSetFlow {
	...
	public static Flow getUserSetFlow(String infile, String outfile) {
		...
		Tap userSetTap = new Hfs(outputSehema, outputPath + "/userSet",
				SinkMode.REPLACE);

		Pipe pipe = new Pipe("userSetPipe");
		...
		Aggregator userAggregator = new userSetAggregator(new Fields("user_id"));
		...
		endPoints.put("userSetPipe", userSetTap);

		Flow flow = flowConnector.connect("userSetModel", source, endPoints,
				pipe);
		return flow;
